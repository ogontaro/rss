...
body > header a, .brand { font-weight: 700; color: var(--fg); text-decoration: none; letter-spacing: .01em; }
.crumb {
  font-size: .78rem;
  font-weight: 600;
  color: var(--accent);
...
/* ---- dashboard ---- */
.cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(230px, 1fr)); gap: 1rem; align-items: start; }
.card {
